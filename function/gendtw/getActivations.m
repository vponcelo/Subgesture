function detSw = getActivations(pred, gt, segs, params)
...
if params.classification
    ...
    for i=2:length(segs),
        if i < length(segs)
            cuseg=pred(iidx:segs(i)-1);
        else
            cuseg=pred(iidx:segs(i));
        end
        if sum(cuseg==1)/length(cuseg) >  params.minOverlap,
            detSw(i-1)=1;
        end
        iidx = segs(i);
    end
    ...
    if ~isempty(idxL)
        ...
        for l = 1:2:length(idxL)        % treat activations
            ...
            if sLabel > 0
                ...
                if sum(GTfr & Predfr)/sum(GTfr | Predfr) > params.minOverlap
                    detSw(iDetL) = 1;
                end
            end
        end
